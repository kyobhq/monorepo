<script lang="ts">
	...
	let { active, action, reverse, label }: Props = $props();
</script>

<label
	class={[
		'flex items-center gap-x-3 transition-colors duration-75',
		active ? 'text-main-50' : 'text-main-500'
	]}
>
	<Switch.Root
		checked={active}
		onCheckedChange={(checked) => (checked ? action() : reverse())}
		class="data-[state=checked]:bg-accent data-[state=unchecked]:bg-main-800 peer border-main-700 data-[state=checked]:border-accent-lighter inline-flex h-[24px] w-[55px] shrink-0 cursor-pointer items-center border !p-0 px-[3px] transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-hidden disabled:cursor-not-allowed disabled:opacity-50"
	>
		<Switch.Thumb
