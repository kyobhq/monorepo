	...
	>
		<ul class="flex flex-col">
			{#each facts as fact, index (index)}
				<li class={['w-full flex gap-x-1 mb-2', facts.length === 3 && 'last:mb-0']}>
					<Select.Root
						type="single"
						value={fact.label}
					...
					>
						<Select.Trigger
							class="bg-main-950 border-[0.5px] border-main-700 w-full px-2 py-1 hover:bg-main-800/50 transition-colors duration-100 text-left hover:cursor-pointer rounded-r-[2px] rounded-l-sm"
						>
							{fact.label || 'Select...'}
						</Select.Trigger>
						<Select.Content
