	...
	const { form, errors, enhance } = superForm(defaults(valibot(EditAvatarSchema)), {
		...
		async onUpdate({ form }) {
			if (form.valid) {
				...
				const res = await backend.updateAvatarAndBanner(
					cropAvatarPixels,
					cropBannerPixels,
					form.data
				);
				res.match(
					(res) => {
						...
						if (res.avatar) userStore.user.avatar = res.avatar;
						if (res.banner) userStore.user.banner = res.banner;
					},
					(error) => {
...
<form method="post" use:enhance class="w-full relative mt-6" enctype="multipart/form-data">
		...
		<div class="relative h-[224px] w-[320px]">
			...
			{:else}
				<input
					...
					class="absolute inset-0 text-transparent peer z-[1] hover:cursor-pointer"
				/>
	...
	{#if initialized && changes}
		<SaveBar
			text="It seems your avatar/banner is not saved yet!"
			bind:isSubmitting
