...
export class WebsocketStore {
  ...
  private async handleMessage(wsMess: WSMessage) {
    ...
    const handlers: Record<string, () => void> = {
      ...
      memberChange: () => this.handleMemberChange(content.value as MemberChange)
    };
  ...
  private handleMemberChange(value: MemberChange) {
    if (!value.userId) return;

    serverStore.editMember(value.serverId, value.userId, value.displayName, value.avatarUrl);
  }
