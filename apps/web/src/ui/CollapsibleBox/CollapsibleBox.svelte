<script lang="ts">
  ...
  const { children, header, canCollapse = true, color }: Props = $props();
...
<div
  class="border-[0.5px] border-main-700"
  style="
...
>
  <button
    ...
    class={[
      "w-full flex items-center justify-start py-1.5 px-2.5 text-main-300 text-sm",
      !isCollapsed && "border-b-[0.5px]",
      canCollapse &&
        "hover:cursor-pointer hocus:bg-main-900 transition-colors duration-75",
    ]}
  >
    {header}
  </button>
